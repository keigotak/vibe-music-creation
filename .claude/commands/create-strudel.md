...
#### 2f. エフェクト/テクスチャ
- フィルタースイープ: `sine.range().slow()` でLFO的変化（**範囲は控えめに**）
- リバーブ/ディレイ: `.room()`, `.delay()`（全パートに深くかけない。ベースにはかけない）
- Lo-Fi感: `.coarse()`, `.crush()` を控えめに（crush 10-13が目安）
- パンニング: `.pan(sine.slow())` で動き

#### 2g. ヒューマナイズ/グルーヴ（打ち込みの質を決める）
- ハイハット等の `.gain()` は必ずパターンで強弱を付ける（固定値は平板になる）
- Lo-Fi/Hip Hop系: スネアに `.nudge(.01〜.02)` でレイドバック
- `perlin.range()` で gain/lpf に微妙な揺らぎ
- 長周期の展開を必ず入れる: `firstOf`/`lastOf`/`degradeBy(sine.slow(16))` 等で16サイクル以上の周期変化 + 稀な変化（Unique Events）を1つ以上

### Step 3: 設計の検証（出力前に必ず行う）
1. **ハーモニー整合表**: 各サイクルスロットの「コード / ベース音 / メロディ着地音」を表にして、ベース=ルート/コードトーン、メロディ着地=コードトーンを確認
2. **`<>` の要素数**: 全パターンでコード進行の小節数と揃っているか確認（意図的なポリメーターを除く）
3. **ゲイン設計**: 同時に鳴る実効gain合計 ≦ 1.5 を目安に配分
   （ドラム合計 0.7-1.0 / ベース 0.5-0.7 / コード 0.2-0.35 / メロディ 0.3-0.5 / テクスチャ 0.1以下）

### Step 4: 出力 & レビュー
1. コードを `strudel/<説明的な名前>.js` に保存（冒頭コメントにキー・BPM・進行・展開周期を記載）
2. 各パターンにコメントで説明を付ける
3. **静的チェック**: `python3 scripts/lint_strudel.py strudel/<ファイル名>.js` を実行し、🔴 を修正してから次へ
4. **strudel-reviewer エージェントでセルフレビュー**: ファイルパスを渡して起動。🔴 Critical は必ず修正して再レビュー（🔴が無くなるまで）、🟡は判断して適用
5. `strudel/README.md` のカタログ表に1行追加する
6. 完成コードをコードブロックで出力し、strudel.cc にペーストする手順を案内
7. カスタマイズのヒントを提案（ミュート方法 `_$:`、パラメータ変更箇所など）
