...
| 2026-07-08 | Strudel→AbletonブリッジをAbletonMCP側でなく本リポジトリの scripts/ + 既存 osc_send で実装 | MCPサーバー変更はClaude Code再起動が必要で検証しづらい。パーサーを独立スクリプトにすればオフラインでテスト可能 | AbletonMCPにMCPツール追加 |
